...
public static class ModProfile
{
    ...
    /// </summary>
    /// <summary>
    ///     是否允许使用除正版以外的验证方式（需先进行一次正版验证）
    /// </summary>
    public static bool CanUseThirdPartyLogin()
    {
#if DEBUG || DEBUGCI
        return true;
#else
        ...
        return hasMinecraftAccount || restricted || !hasNetwork;
#endif
    }
    ...
    public static void CreateProfile()
    {
        ...
        ModBase.RunInUiWait(() =>
        {
            var authTypeList = _GetAvailableProfileSelection(CanUseThirdPartyLogin());
            ...
            ModBase.RunInNewThread(() =>
            {
                ...
                catch (HttpRequestException ex)
                {
                    ...
                    if (exSummary.Contains("403"))
                        ModMain.MyMsgBox(Lang.Text("Launch.Account.Profile.EditPlayerId.Cooldown"), Lang.Text("Launch.Account.Profile.EditPlayerId.Failed.Title"), Lang.Text("Common.Action.Confirm"), isWarn: true);
                    else
                        ModBase.Log(
                            ex,
